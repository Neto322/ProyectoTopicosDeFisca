...
namespace MerryYellow.AutoInputManager.UnityEnchanterGUI
{
    ...
    static class MyTreeElementGenerator
	{
        ...
        static int GetInsertionIndex(List<MyTreeElement> treeElements, MerryYellow.CodeEnchanter.Common.MyDocument document, int enchanterCount)
        {
            ...
            {
                var topFolders = string.Join("\\", folders, 0, i);
                var curFolders = string.Join("\\", folders, 0, i + 1);
                var index = treeElements.FindLastIndex(e => e.folders != null && e.folders.StartsWith(curFolders));
                ...
                else
                {
                    while (lastValidIndex > 0 && treeElements[lastValidIndex].type == MyTreeElement.TypeDocument &&
                        treeElements[lastValidIndex].folders == topFolders)
                        lastValidIndex--;

                    var folderNode = new MyTreeElement(folders[i], 1 + i, ++IDCounter, null, enchanterCount, MyTreeElement.TypeFolder, curFolders);
        ...
        public static List<MyTreeElement> GenerateTree(MerryYellow.CodeEnchanter.Common.MySolution solution, int enchanterCount)
        {
            ...
            var root = new MyTreeElement(solution.path, -1, IDCounter, null, enchanterCount, MyTreeElement.TypeRoot, null);
            treeElements.Add(root);

            foreach(var project in solution.Projects)
            {
                var projectNode = new MyTreeElement(project.Name, 0, ++IDCounter, null, enchanterCount, MyTreeElement.TypeProject, project.Folders);
                ...
                foreach (var document in project.Documents)
                {
                    ...
                    var documentNode = new MyTreeElement(document.Name, 1 + folders.Length, ++IDCounter, document.Name, enchanterCount, MyTreeElement.TypeDocument, document.Folders);
        ...
        public static List<MyTreeElement> GenerateRandomTree(int numTotalElements, int enchanterCount)
		{
			...
			var root = new MyTreeElement("Root", -1, IDCounter, null, enchanterCount, MyTreeElement.TypeRoot, null);
			treeElements.Add(root);
		...
		static void AddChildrenRecursive(TreeElement element, int numChildren, bool force, int numTotalElements, ref int allowedDepth, List<MyTreeElement> treeElements, int enchanterCount)
		{
			...
			for (int i = 0; i < numChildren; ++i)
			{
				...
				var child = new MyTreeElement("Element " + IDCounter, element.depth + 1, ++IDCounter, null, enchanterCount, MyTreeElement.TypeDocument, null);
